...
Microsoft_refs = ['Microsoft', 'MSFT', '$MSFT']
...
df['Microsoft'] = df['Tweets'].apply(
    lambda x: identify_subject(x, Microsoft_refs))
...
df[df['Microsoft'] == 1][['Microsoft', 'polarity', 'subjectivity']].groupby(
    'Microsoft').agg([np.mean, np.max, np.min, np.median])

Microsoft = df[df['Microsoft'] == 1][['Timestamp', 'polarity']]
Microsoft = Microsoft.sort_values(by='Timestamp', ascending=True)
Microsoft['MA Polarity'] = Microsoft.polarity.rolling(
    10, min_periods=10).mean()


df = df[['polarity', 'subjectivity']]


df.to_sql('microsoft', connection, if_exists='append', index=False)
